...
<body>
	...
	<table border=1>
		...
		ArrayList<CONTENTDTO> list = cd.getList(paging, field, query);//게시글 목록조회
		...
		pageContext.setAttribute("field", field);//검색 조건 선택 (title, userID)
		...
		<c:forEach var="l" items="${list}" varStatus="st">
			<tr>
				<td>${l.contentID}</td>
				<td><a
					href="view.jsp?contentID=${l.contentID }&p=${(empty param.p)?1:param.p}&f=${field}&q=${query}">${l.title}</a></td>
				<td>${l.userID}</td>
				<td>${l.regdate }</td>
				<td>${l.hit}</td>
		</c:forEach>
	...
	<ul>
		<c:forEach var="i" begin="0" end="4">
			...
			<c:if test="${(startNum+i) <= lastNum}">
				<!-- 조냊하는 글 갯수만큼만 페이지 표현 -->
				<li><a id="num" style="${style}"
					href="?p=${startNum+i}&f=${field}&q=${query}">${startNum+i }</a></li>
			</c:if>
		</c:forEach>
	</ul>
